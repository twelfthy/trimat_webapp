...
start = time.time()
struct_format = 'i' + 'i' * (16*16)
# Function to update the heatmap data
def update_heatmap(frame):
    global matrix
    # Read the 8x8 matrix from Arduino
    #matrix_str = ser.readline().decode().strip()
    new_matrix = ser.read(struct.calcsize(struct_format)) #np.array([int(value) for value in matrix_str.split(',')])
    sensor_value, *matrix_values = struct.unpack(struct_format, new_matrix)
    matrix = [matrix_values [i: i + 16 ] for i in range(0, len(matrix_values), 16)]
    #print(matrix)
    # Update the individual elements of the matrix with the new values
    matrix = np.reshape(matrix, (16,16))
    end = time.time()-start
    print(end)
    # print(matrix)
    # start=end
